...
Enables support for SSL/TLS compression, same as clearing
B<SSL_OP_NO_CERTIFICATE_COMPRESSION>.
This command was introduced in OpenSSL 3.0.0.
...
B<CertificateCompression>: use the certificate compression extension, enabled by
default. Inverse of B<SSL_OP_NO_CERTIFICATE_COMPRESSION>: that is,
B<-CertificateCompression> is the same as setting B<SSL_OP_NO_CERTIFICATE_COMPRESSION>.
