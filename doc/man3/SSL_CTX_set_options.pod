...
=item SSL_OP_NO_CERTIFICATE_COMPRESSION

Normally clients and servers will transparently attempt to negotiate the
draft-ietf-tls-certificate-compression option on TLSv1.3 connections.

If this option is set, certificate compression is disabled. The extension
will not be sent by either client or server, and will be ignored upon
receipt.
