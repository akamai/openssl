...
/* Note: There is also a linux zstd file in the kernel source */
# ifndef ZSTD_H_235446
#  error WRONG ZSTD?
# endif

# if ZSTD_VERSION_MAJOR != 1 && ZSTD_VERSION_MINOR != 4
#  error Expecting version 1.4.x of ZSTD
# endif
...
/* Function pointers */
typedef ZSTD_CStream* (*createCStream_ft)();
...
typedef ZSTD_DStream* (*createDStream_ft)();
...
#  endif
    ...
    if (p_createCStream == NULL || p_initCStream == NULL || p_freeCStream == NULL
            ...
            || p_isError == NULL
            || !OPENSSL_init_crypto(OPENSSL_INIT_ZSTD, NULL)) {
...
static int bio_zstd_write(BIO *b, const char *in, int inl)
{
    ...
    for (;;) {
        ...
        while (ctx->compress.write_pos < ctx->compress.outbuf.pos) {
            ret = BIO_write(next, ctx->compress.outbuf.dst + ctx->compress.write_pos,
